...
export const DisplayOptions = ({
	options,
	switchZoom,
	switchBordersVisibility,
	clearCanvas,
	changeCanvasSize,
}: DisplayOptionsProp) => {
	const [auxCanvasSize, setAuxCanvasSize] = React.useState<number>(
		options.canvasSize,
	);

	return (
		<div className="DisplayOptions">
			<button onClick={() => switchZoom()}>zoom</button>
			<button onClick={() => switchBordersVisibility()}>borders</button>
			<label htmlFor="matrix_size">
				<input
					...
					onChange={(event) =>
						setAuxCanvasSize(Number(event.currentTarget.value))
					}
					value={auxCanvasSize}
				/>
				<strong
					className={
						auxCanvasSize !== options.canvasSize
