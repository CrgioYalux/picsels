...
export const App = () => {
  ...
  const [options, setOptions] = React.useState<Tools>({zoom:false, bordersVisibility: true, canvasSize: 5});
  const [matrix, setMatrix] = React.useState<(string|number)[][]>(() => createMatrix(options.canvasSize));
  ...
  const paintElement = ({x,y}:{x:number, y:number}) => {
      setMatrix(prev => {
          const _prev = [...Array(options.canvasSize)].map((_, index) => [...prev[index]]);
  ...
  const switchZoom = () => {
    setOptions(prev => ({zoom:!prev.zoom, bordersVisibility: prev.bordersVisibility, canvasSize: prev.canvasSize}))
  }
  ...
  const changeCanvasSize = (event:React.SyntheticEvent<HTMLInputElement>) => {
    ...
    if (canvasSize !== options.canvasSize) {
      setOptions(prev => ({zoom:prev.zoom, bordersVisibility:prev.bordersVisibility, canvasSize}))
    }
  }
  ...
  return (
    <div className="App">
      ...
      <DisplayOptions options={options} changeCanvasSize={changeCanvasSize} clearCanvas={clearCanvas} switchZoom={switchZoom} switchBordersVisibility={switchBordersVisibility} />
